...
func (ms *MachinePlugin) CreateMachine(ctx context.Context, req *driver.CreateMachineRequest) (*driver.CreateMachineResponse, error) {
	...
	providerSpec, err := decodeProviderSpecAndValidate(req.MachineClass, req.Secret)
	if err != nil {
		return nil, prepareErrorf(err, "Create machine %q failed on decodeProviderSpecAndValidate", req.Machine.Name)
	}
...
func (ms *MachinePlugin) GetMachineStatus(ctx context.Context, req *driver.GetMachineStatusRequest) (*driver.GetMachineStatusResponse, error) {
	...
	providerSpec, err := decodeProviderSpecAndValidate(req.MachineClass, req.Secret)
	if err != nil {
		return nil, prepareErrorf(err, "Machine status %q failed on decodeProviderSpecAndValidate", req.Machine.Name)
	}
...
func (ms *MachinePlugin) ListMachines(ctx context.Context, req *driver.ListMachinesRequest) (*driver.ListMachinesResponse, error) {
	...
	providerSpec, err := decodeProviderSpecAndValidate(req.MachineClass, req.Secret)
	if err != nil {
		return nil, prepareErrorf(err, "List machines failed on decodeProviderSpecAndValidate")
	}
